...
const REPORTS_MANUAL_SEND_EMAILS = ['info@strefaciszy.net'];
...
exports.listUsersHttp = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    return res.json(results.filter((u) => u.email && u.email.toString().trim() !== ''));
  } catch (e) {
...
exports.createUserHttp = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    await admin.firestore().collection('users').doc(user.uid).set({name, email, role});
...
async function _appendWorkDaySheet({workbook, db, dayKey}) {
  ...
  const summaryHeaderRow = sheet.addRow(['Imię', 'Nazwisko', 'Data', 'Suma godzin']);
  ...
  for (const s of summaryRows) {
    sheet.addRow([s.firstName, s.lastName, s.date, _formatMinutesAsHoursPl(s.totalMinutes)]);
  }
  ...
  try {
    ...
    if (sheet) {
      for (const {ref, data: dData, createdAt} of docsForDay) {
        ...
        if (items.length === 0 && notesRaw.length === 0 && doneTasks.length === 0) {
        ...
        const doneTaskTexts = new Set(doneTasks.map((t) => (t.text || '').trim().toLowerCase()));
...
//         if (docsForDay.length === 0 && workDayCountForDay === 0) {
//           console.log(
//               `[RW scheduled] Brak zapisanych dokumentów RW ani MÓJ DZIEŃ ${dayKey} – nie wysyłam maila.`,
//           );
//           return;
//         }
...
//         console.log('[RW scheduled] Report sent OK', {
//           dayKey,
//           to: reportsTo,
//           bcc: reportsBcc,
//           count: docsForDay.length,
...
async function getUserTokens(uid) {
  const snap = await admin.firestore().collection('users').doc(uid).collection('push_tokens').get();
...
function _warsawDayKey(date = new Date()) {
...
async function _buildReadableBackupWorkbook(dayKey) {
  const db = admin.firestore();
  const workbook = new ExcelJS.Workbook();
  ...
  projectsSheet.columns = [
    ...
    {header: 'Project', key: 'projectName', width: 35},
  ...
  for (const customerDoc of customersSnap.docs) {
    ...
    const projectsSnap = await customerDoc.ref.collection('projects').get();

    for (const projectDoc of projectsSnap.docs) {
      const p = projectDoc.data() || {};
      const updatedAt = p.updatedAt && p.updatedAt.toDate ? p.updatedAt.toDate().toISOString() : '';
      projectsSheet.addRow({
        customerId: customerDoc.id,
        customerName: customer.name || customer.customerName || '',
        projectId: projectDoc.id,
        projectName: p.name || p.projectName || '',
        archived: p.archived === true ? 'YES' : 'NO',
        updatedAt,
      });
    }
  }
  ...
  // RW DOCUMENTS
  const rwSheet = workbook.addWorksheet('RW Documents');
  rwSheet.columns = [
  ...
  const rwSnap = await db.collectionGroup('rw_documents').get();
  ...
  rwSnap.forEach((doc) => {
    ...
    rwSheet.addRow({
      ...
      user: d.createdByName || d.createdBy || '',
      type: d.type || 'RW',
      itemsCount: Array.isArray(d.items) ? d.items.length : 0,
      notesCount: Array.isArray(d.notesList) ? d.notesList.length : 0,
      path: doc.ref.path,
    });
  });

  // WORK DAY LOGS
  const workSheet = workbook.addWorksheet('Work Day Logs');
  workSheet.columns = [
    {header: 'Day', key: 'dayKey', width: 14},
    {header: 'User', key: 'userName', width: 24},
    {header: 'Start', key: 'startTime', width: 12},
    {header: 'End', key: 'endTime', width: 12},
    {header: 'Minutes', key: 'durationMinutes', width: 12},
    {header: 'Project', key: 'projectName', width: 35},
    {header: 'Description', key: 'description', width: 60},
  ];
  workSheet.getRow(1).font = {bold: true};

  const workSnap = await db.collection('work_day_logs').get();
...
async function _deleteOldReadableBackups() {
  const bucket = admin.storage().bucket();
  const [files] = await bucket.getFiles({prefix: 'backups/readable/'});

  const folders = new Set();

  for (const file of files) {
    const parts = file.name.split('/');
    if (parts.length >= 3 && parts[0] === 'backups' && parts[1] === 'readable') {
      folders.add(parts[2]);
    }
  }
  ...
  for (const file of files) {
    ...
    if (folder && !keep.has(folder)) {
      await file.delete().catch(() => {});
    }
  }
}

exports.createReadableBackupScheduled = onSchedule(
    {
      schedule: '30 23 * * *',
      timeZone: 'Europe/Warsaw',
    },
    async () => {
      const dayKey = _warsawDayKey();
      const bucket = admin.storage().bucket();

      const buffer = await _buildReadableBackupWorkbook(dayKey);
      const filePath = `backups/readable/${dayKey}/readable_backup_${dayKey}.xlsx`;
      ...
      await _deleteOldReadableBackups();

      console.log('[BACKUP] readable backup created', {dayKey, filePath});
    },
);

// BACKUPS

exports.downloadReadableBackupHttp = functions.https.onRequest(async (req, res) => {
  if (req.method === 'OPTIONS') {
    return res.status(204).set(corsHeaders).send('');
  }
  res.set(corsHeaders);
  ...
  const dayKey = req.body && req.body.dayKey ? req.body.dayKey : null;
  if (!dayKey || typeof dayKey !== 'string') {
  ...
  try {
    ...
    if (!exists) {
      return res.status(404).json({error: `No readable backup found for ${dayKey}`});
    }
    ...
    res.setHeader('Content-Disposition', `attachment; filename="readable_backup_${dayKey}.xlsx"`);

    return res.status(200).send(buffer);
  } catch (err) {
    console.error('downloadReadableBackupHttp error', err);
    return res.status(500).json({
      error: 'Internal error',
      details: err && err.message ? err.message : String(err),
    });
...
exports.createAndDownloadReadableBackupHttp = functions.https.onRequest(async (req, res) => {
  if (req.method === 'OPTIONS') {
    return res.status(204).set(corsHeaders).send('');
  }
  res.set(corsHeaders);
  ...
  try {
    ...
    res.setHeader(
        'Content-Disposition',
        `attachment; filename="manual_readable_backup_${dayKey}.xlsx"`,
    );

    return res.status(200).send(Buffer.from(buffer));
  } catch (err) {
    console.error('createAndDownloadReadableBackupHttp error', err);
    return res.status(500).json({
      error: 'Internal error',
      details: err && err.message ? err.message : String(err),
    });
