...
class StockService {
  ...
  static Map<String, dynamic> buildRwDocMap(
    ...
    String customerId, // ← new parameter
  ) {
    // stamp out just the date (UTC midnight) so your queries can be single-field
    ...
    return {
      ...
      'customerId': customerId, // ← store it here
      'createdBy': createdBy,
      'createdAt': createdAt,
      'createdDay': Timestamp.fromDate(createdDay), // ← add for easy querying
      'type': type,
      'items': lines.map((l) {
        /* … */
      }).toList(),
