...
class StockService {
  ...
  static Future<void> applyProjectLinesTransaction({
    ...
    return db.runTransaction((tx) async {
      // 1) Load existing RW items (if any)
      final rwSnapshot = await tx.get(rwRef);
      final oldData = rwSnapshot.exists
          ? rwSnapshot.data()!['items'] as List<dynamic>
          : <dynamic>[];
      final oldLines = oldData
          .map((m) => ProjectLine.fromMap(Map<String, dynamic>.from(m)))
          .where((ln) => ln.isStock)
          .toList();

      // 2) Apply deltas for updated or new lines
      for (final ln in lines.where((l) => l.isStock)) {
        await _applyLineDelta(tx, ln, userId);
      }

      // 3) Restore stock for any lines removed in the editor
      for (final oldLn in oldLines) {
        final stillPresent = lines.any(
          (l) => l.isStock && l.itemRef == oldLn.itemRef,
        );
        if (!stillPresent) {
          // simulate requestedQty → 0 to fully restore stock
          final restoreLn = oldLn.copyWith(requestedQty: 0);
          await _applyLineDelta(tx, restoreLn, userId);
        }
      }

      // 4) Create or update the RW document
      if (isNew) {
