                ...
                children: [
                  ...
                  PhotoGallery(
                    ...
                    onAddImage: (source) async {
                      ...
                      final url = await _storage.pickAndUploadProjectImage(
                        widget.projectId,
                        source,
                      );

                      if (url != null) {
                        // 1) Replace the placeholder with the real URL
                        final idx = _imageUrls.indexOf(placeholder);
                        setState(() => _imageUrls[idx] = url);
                        ...
                        setState(() => _imageUrls.remove(placeholder));
                      }
                  ...
                  NotesSection(
                    notes: _notes,
                    onAddNote: (ctx) async {
                      // 1) Ask the user for the note text:
                      String draft = '';
                      final result = await showDialog<String>(
                        context: ctx,
                      ...
                      // 2) Write it to Firestore (no local setState!)
                      final user = FirebaseAuth.instance.currentUser!;
                      ...
                      // 3) Return null so we don’t do any local insert
                      return null;
                    },
                    onDelete: (i) async {
                      final note = _notes[i];
