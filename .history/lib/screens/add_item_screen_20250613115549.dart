...
class _AddItemScreenState extends State<AddItemScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // 1) Create the document to get its real ID
      ...
      // 2) If the user picked an image, upload it once and then get its URL
      if (_pickedImage != null) {
        final path = 'stock_images/${docRef.id}.jpg';
        print('📤 uploading image to: $path');
        final ref = _storage.child(path);

        await ref.putFile(_pickedImage!); // upload
        print('✅ upload complete, now fetching URL');
        final url = await ref.getDownloadURL(); // download URL

        print('🔗 got download URL: $url');
        await docRef.update({'imageUrl': url}); // write back to Firestore
      }
