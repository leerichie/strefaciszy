...
class FilteredReportScreen extends StatelessWidget {
  ...
  Future<void> _exportToExcel(
    ...
    // 2) Clean up your project/report name for a file name
    final project =
        (data['projectName'] as String?)
            ?.replaceAll(RegExp(r'\s+'), '_') // spaces → underscores
            .replaceAll(RegExp(r'[^\w\-_\.]'), '') // remove any illegal chars
            ??
        'dokument';

    // 3) Build the filename WITHOUT “.xlsx” – FileSaver will append it
    final filename = '${project}_$formattedDate';
    ...
    if (savedPath != null) {
      ...
      await Share.shareXFiles(
        [XFile(savedPath)],
        text: 'Raport materialowy Strefa Ciszy',
        subject: filename,
      );
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Zapisano i udostępniono: $savedPath')),
      );
    } else {
      // web: just downloaded
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Pobrano plik: $filename')));
    }
  }
}
