...
class FilteredReportScreen extends StatelessWidget {
  ...
  Future<void> _exportToExcel(
    ...
    final data = Uint8List.fromList(bytes); // Uint8List

    // save to disk / download
    final savedPath = await FileSaver.saveFile(
      data,
      filename: 'dokument_${DateTime.now().millisecondsSinceEpoch}',
      mimeType:
          'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    );

    if (savedPath != null) {
      // on mobile/desktop we can pop up a share sheet
      ...
      // on web we’ve already triggered download
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Plik pobrany do przeglądarki')));
    }
  }
}
