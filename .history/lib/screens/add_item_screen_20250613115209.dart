...
class _AddItemScreenState extends State<AddItemScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // 1) create doc to get its real ID
      ...
      print('✅ upload complete, now fetching download URL');
      final url = await ref.getDownloadURL();
      print('🔗 got download URL: $url');
      await docRef.update({'imageUrl': url});

      // 2) if image picked, upload it against that doc ID
      final docRef = await col.add({ … });                                   // 1
if (_pickedImageFile != null) {
  final path = 'stock_images/${docRef.id}.jpg';
  final ref  = _storage.child(path);
  await ref.putFile(_pickedImageFile!);                                // 2
  final url  = await ref.getDownloadURL();                             // 3
  await docRef.update({'imageUrl': url});
}
