...
class _RWDocumentsScreenState extends State<RWDocumentsScreen> {
  ...
  String _buildCsv(Map<String, dynamic> data) {
    ...
    // Then each item line: [nazwa, ilość, jm]
  ...
  Future<void> _copyCsv(Map<String, dynamic> data) async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('CSV skopiowane do schowka')));
  }

  Future<void> _exportCsv(BuildContext ctx, Map<String, dynamic> data) async {
    final csv = _buildCsv(data);
    // Save to a temporary file (on mobile) or share text directly
    final tempDir = await getTemporaryDirectory();
    final file = File('${tempDir.path}/document.csv');
    await file.writeAsString(csv, encoding: utf8);

    // Share via native share sheet
    await Share.shareFiles([file.path], text: 'Eksport CSV dokumentu');
  }
