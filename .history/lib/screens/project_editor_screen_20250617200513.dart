...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _openNotes() async {
    String draft = '';
    final newEntry = await showDialog<String>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Nowa notatka'),
        content: TextField(
          decoration: const InputDecoration(hintText: 'Wpisz notatkę…'),
          maxLines: 5,
          onChanged: (v) => draft = v,
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () =>
                Navigator.pop(ctx, draft.trim().isEmpty ? null : draft.trim()),
    ...
    if (newEntry != null) {
      final ts = DateFormat('dd.MM.yyyy HH:mm', 'pl_PL').format(DateTime.now());
      setState(() {
        _notes = '$newEntry\n$_notes';
      });
    }
                          ...
                          children: [
                            ...
                            Expanded(
                              child: SizedBox(
                                height:
                                    5 *
                                    20.0, // ~5 lines at 20px each; tweak as needed
