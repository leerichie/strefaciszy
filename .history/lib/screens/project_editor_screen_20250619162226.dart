...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _saveRWDocument(String type) async {
    ...
    final bool isNew = existingSnap.docs.isEmpty;
    final String rwId = isNew ? rwCol.doc().id : existingSnap.docs.first.id;

    // 2) only admins may update (isNew==false)
    if (!isNew && !widget.isAdmin) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Nie masz uprawnień do edycji dokumentu z poprzednich dni.',
          ),
        ),
      );
    ...
    // 3) preserve original createdAt/createdBy when updating
    DateTime createdAt = now;
    String createdBy = user.uid;
    if (!isNew) {
      final data = existingSnap.docs.first.data();
      final ts = data['createdAt'];
      createdAt = ts is Timestamp ? ts.toDate() : createdAt;
    ...
    try {
      await StockService.applyProjectLinesTransaction(
        ...
        rwDocId: rwId,
        rwDocData: rwData,
        isNew: isNew, // pass along the new/modify flag
        lines: _lines,
        newStatus: type,
        userId: user.uid,
      );
