...
class _RWDocumentsScreenState extends State<RWDocumentsScreen> {
  ...
  Widget build(BuildContext context) {
      ...
      child: StreamBuilder<QuerySnapshot>(
        ...
        builder: (ctx, snap) {
          ...
          final filtered = docs.where((doc) {
            ...
            final uName = userNames[uid]?.toLowerCase() ?? uid.toLowerCase();
            final proj  = (d['projectName'] ?? '').toString().toLowerCase();

            final textOk = _userFilter.isEmpty ||
          ...
          if (filtered.isEmpty) {
            return const Center(child: Text('Brak zapisanych dokumentów.'));
          }
          ...
          return ListView.builder(
            ...
            itemBuilder: (ctx, i) {
              ...
              final date  = DateFormat('dd.MM.yyyy • HH:mm', 'pl_PL').format(ts);

              final uid = d['createdBy'] as String? ?? '';
              _fetchUserName(uid);

              // check if document is from today
              final startOfDay      = DateTime(ts.year, ts.month, ts.day);
              final startOfTomorrow = startOfDay.add(const Duration(days: 1));
              final now             = DateTime.now();
              final isToday         = now.isAfter(startOfDay) && now.isBefore(startOfTomorrow);
                    ...
                    onPressed: () async {
                      final confirm = await showDialog<bool>(
                        ...
                        builder: (ctx2) => AlertDialog(
                          ...
                          actions: [
                            TextButton(onPressed: () => Navigator.pop(ctx2, false), child: const Text('Anuluj')),
                            ElevatedButton(onPressed: () => Navigator.pop(ctx2, true), child: const Text('Usuń')),
                          ],
                        ),
                      );
                      if (confirm == true) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Dokument usunięty')),
                        );
                      }
                    },
                  ),
    ...
    // ─── History (Bottom Half, only for project view) ─────────────────────────
    if (isClientView) ...[
