...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _saveRWDocument(String type) async {
    ...
    // find or create today's RW doc ID
    final now = DateTime.now();
    final startOfDay = DateTime(now.year, now.month, now.day);
    final startOfTomorrow = startOfDay.add(Duration(days: 1));
    ...
    // check edit-permissions on past docs
    ...
    // 2) Build and write the RW using filteredLines
    ...
    try {
      ...
      // 3) Update local list so zero‐qty items vanish immediately
      setState(() {
