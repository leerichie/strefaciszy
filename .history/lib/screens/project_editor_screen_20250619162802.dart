...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _saveRWDocument(String type) async {
    ...
    // Look for an existing RW of this type created *today*
    ...
    // If an RW already exists *but* its createdAt is *before* today, that's a closed RW
    final rwRef = rwCol.doc(rwId);
    final docSnap = await rwRef.get();
    if (docSnap.exists) {
      final data = docSnap.data()!;
      final rawTs = data['createdAt'];
      DateTime createdAtRaw = rawTs is Timestamp ? rawTs.toDate() : now;
      if (createdAtRaw.isBefore(startOfDay) && !widget.isAdmin) {
        // Non-admins may NOT update a closed (past) RW
    ...
    // Determine correct createdBy/createdAt
