...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  /// Builds color-coded preview rows showing remaining stock in brackets
  List<Widget> _buildPreviewRows() {
    return _lines.map((ln) {
      final isStock = ln.isStock;
      final name = isStock
          ? _stockItems
                .firstWhere(
                  (s) => s.id == ln.itemRef,
      ...
      final display = isStock ? '($remaining/${totalStock})' : '';
      final enough = !isStock || remaining >= 0;
      return Padding(
  ...
  Widget _buildCompactRow(int index) {
    ...
    return ListTile(
      ...
      subtitle: Text('${ln.requestedQty} ${ln.unit}'),
      trailing: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
