...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _addLine() async {
    final ProjectLine? newLine = await showDialog<ProjectLine>(
      context: context,
      builder: (ctx) {
        bool isStock = true;
        String itemRef = '';
  ...
  List<Widget> _buildPreview() {
    return _lines.map((ln) {
      if (ln.isStock) {
        if (ln.itemRef.isEmpty) return SizedBox.shrink();
        ...
        return Text(
          'Order: \${ln.customName} – \${ln.requestedQty} \${ln.unit}',
        );
      }
        ...
        child: Row(
          children: [
            DropdownButton<bool>(
              ...
              onChanged: (v) => setState(() => line.isStock = v!),
            ),
            SizedBox(width: 8),
            ...
            Expanded(
              child: line.isStock
                  ? DropdownButtonFormField<String?>(
                      value: line.itemRef.isNotEmpty ? line.itemRef : null,
                      items: _stockItems
                          .map(
                            (s) => DropdownMenuItem(
                              value: s.id,
                          ...
                          .toList(),
                      onChanged: (v) => setState(() => line.itemRef = v ?? ''),
                      decoration: InputDecoration(labelText: 'Item'),
                      validator: (v) {
                        if (line.isStock && (v == null || v.isEmpty))
                          return 'Required';
                        return null;
                      },
                    )
