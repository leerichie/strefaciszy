...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _saveRWDocument(String type) async {
    ...
    final currentStatus = projectSnap.data()?['status'] as String? ?? '(none)';
    debugPrint(
      '🔄 _saveRWDocument called; currentStatus="$currentStatus", type="$type"',
    );

    // TEMPORARILY comment out the early bail-out:
    /*
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (ctx, snap) {
          ...
          final urls = (data['images'] as List<dynamic>?)?.cast<String>() ?? [];
          _images = urls.map((u) => XFile(u)).toList();
