  ...
  Future<void> _saveRWDocument(String type) async {
    ...
    try {
      ...
      if (filteredLines.isEmpty && docSnap.exists) {
        ...
        // ➤ 3. Optional: also remove lines from Firestore if stored in project
        final projectRef = FirebaseFirestore.instance
            .collection('customers')
            .doc(widget.customerId)
            .collection('projects')
        ...
        await projectRef.update({
          'lines': FieldValue.arrayRemove(
            oldLines.map((l) => l.toMap()).toList(),
          ),
        });
