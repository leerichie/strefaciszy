...
class _RWDocumentsScreenState extends State<RWDocumentsScreen> {
  ...
  Future<void> _exportToExcel(
    ...
    // Section header
    final startRow = 4;
    sheet.getRangeByName('A$startRow').setText('Nazwa materiału');
    ...
    for (final item in (data['items'] as List<dynamic>? ?? [])) {
      ...
      sheet.getRangeByName('B$row').setText('${item['quantity'] ?? ''}');
      row++;
    }
    ...
    final filename = '${safeProj}_$stamp.xlsx';
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Zapisano: ${file.path}')));
