...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _saveRWDocument(String type) async {
    ...
    for (var ln in filteredLines) {
      if (ln.previousQty == null || ln.previousQty == 0) {
    ...
    try {
      ...
      setState(() {
        _lines = _lines.map((l) {
          final matched = filteredLines.firstWhere(
            (f) => f.itemRef == l.itemRef,
            orElse: () => l,
          );
          return l.itemRef == matched.itemRef
              ? l.copyWith(previousQty: matched.previousQty)
              : l;
        }).toList();
      });
