...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _loadAll() async {
    ...
    // ① If we're editing an existing RW, grab its own `items` array and map it to lines:
    if (widget.rwId != null) {
      final rwSnap = await projRef
          ...
          .get();
      if (rwSnap.exists) {
        final data = rwSnap.data()! as Map<String, dynamic>;
        final rawItems = data['items'] as List<dynamic>? ?? [];
        _lines = rawItems.map((e) {
          final m = e as Map<String, dynamic>;
          // build a ProjectLine with requestedQty == previousQty
          return ProjectLine(
            itemRef: m['itemId'] as String,
            requestedQty: (m['quantity'] as num).toInt(),
            previousQty: (m['quantity'] as num).toInt(),
            unit: m['unit'] as String,
            // detect stock vs custom by looking up in your stock list:
            isStock: _stockItems.any((s) => s.id == m['itemId']),
            customName: m['name'] as String,
          );
        }).toList();
    ...
    // ② Otherwise fall back to your original project-draft logic:
