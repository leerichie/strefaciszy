...
class _RWDocumentsScreenState extends State<RWDocumentsScreen> {
  ...
  String _buildCsv(Map<String, dynamic> data) {
    ...
    // 3) Spacer row (all empty)
    final spacer = ['', '', '', ''];

    // 4) Section title row
    final sectionTitle = ['Materiały', '', '', ''];

    // 5) Materials header (3 columns + blank)
    final materialHeader = ['Nazwa', 'Ilość', 'Jm', ''];

    // 6) Material lines (each with 4 columns)
    final items = (data['items'] as List<dynamic>? ?? []).map<List<dynamic>>((
      it,
    ) {
      return [
        it['name'] ?? '',
  ...
  Future<void> _copyCsv(Map<String, dynamic> data) async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('skopiowane do schowka')));
  }

  Future<void> _exportCsv(BuildContext ctx, Map<String, dynamic> data) async {
    final csv = _buildCsv(data);
    final tempDir = await getTemporaryDirectory();
    final filePath = '${tempDir.path}/document.csv';
    final file = File(filePath);
    await file.writeAsString(csv, encoding: utf8);

    final xfile = XFile(filePath, mimeType: 'text/csv');
    await Share.shareXFiles([xfile], text: 'Eksport do CSV');
  }
