...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  // ─── ① if we're editing an existing RW doc, load *that* doc's items ───
  if (widget.rwId != null) {
    ...
    if (rwSnap.exists) {
      ...
      // now map each RW-item to your ProjectLine, supplying originalStock:
      _lines = rawItems.map((e) {
        ...
        // **here** we pull the current stock quantity from your cached list:
        ...
        return ProjectLine(
          ...
          previousQty:   qty,             // "synced" with the doc
          unit:          unit,
        );
      }).toList();
  ...
  // ─── ② otherwise fall back to your existing project‐items logic ───
  final snap = await projRef.get();
  final data = snap.data()!;
  final lastRwRaw = data['lastRwDate'];
  DateTime? lastRwDate =
      lastRwRaw is Timestamp ? lastRwRaw.toDate() : null;
  final now = DateTime.now();
