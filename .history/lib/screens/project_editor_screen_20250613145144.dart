    ...
    return showDialog<ProjectLine>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setState) {
          // filter stockItems by searchQuery
                      ...
                      TextFormField(
                        decoration: InputDecoration(
                          labelText: itemRef.isEmpty
                              ? 'Search item'
                              : 'Item: ${_stockItems.firstWhere((s) => s.id == itemRef).name}',
    ...
    builder: (ctx) => StatefulBuilder(
      builder: (ctx, setState) {
        ...
        return AlertDialog(
          title: Text(existing == null ? 'Add Line' : 'Edit Line'),
          content: Container(
            width: double.maxFinite, // allow the dialog to fill available width
            child: Form(
              key: _formKey,
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  ...
                  // 2a) STOCK: show search + live list
                  if (isStock) ...[
                    TextFormField(
                      decoration: InputDecoration(
                        labelText: itemRef.isEmpty
                          ? 'Search item'
                          : 'Selected: ' + _stockItems
                    ...
                    // only if typing and nothing selected yet
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              child: Text(existing == null ? 'Add' : 'Save'),
                    ...
                    TextFormField(
                      ...
                      keyboardType: TextInputType.number,
                      onChanged: (v) =>
                          requestedQty = int.tryParse(v) ?? requestedQty,
                      validator: (v) {
                        final n = int.tryParse(v ?? '');
                        return (n == null || n < 0) ? 'Invalid' : null;
                      },
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () {
                  // ensure stock item was picked, if stock mode
                  if (!_dlgFormKey.currentState!.validate()) return;
                  if (isStock && itemRef.isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('Please select an item.')),
                    );
