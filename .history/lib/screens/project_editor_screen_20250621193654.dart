...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _saveRWDocument(String type) async {
    ...
    if (filteredLines.isEmpty) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Brak produkty do RW')));
      return;
    }
  ...
  await rwRef.delete();
  print('🗑️ RW deleted: $rwRef');

  for (final line in _lines.where((l) => l.isStock && l.previousQty > 0)) {
    final stockItem = _stockItems.firstWhere((s) => s.id == line.itemRef);
    print('♻️ Restoring ${line.previousQty} to ${stockItem.name} (${stockItem.id})');
    await StockService.increaseQty(stockItem.id, line.previousQty);
  }

  // 👇 NEW: Actually remove items from the project
  await projectRef.update({
    'items': [],
    'lastRwDate': FieldValue.serverTimestamp(),
  });
  print('🧹 Project items cleared for ${widget.projectId}');
