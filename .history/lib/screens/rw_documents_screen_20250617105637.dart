...
class _RWDocumentsScreenState extends State<RWDocumentsScreen> {
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (ctx, snap) {
          ...
          final filtered = docs.where((doc) {
            ...
            final projectName = (d['projectName'] ?? '')
                .toString()
                .toLowerCase();

            final textMatch =
                _userFilter.isEmpty ||
                userName.contains(_userFilter) ||
                projectName.contains(_userFilter);
            ...
            return typeMatch && textMatch && fromOk && toOk;
          }).toList();
          if (filtered.isEmpty) {
  ...
  void _applyFilter() {
    final f = _userFilter.trim().toLowerCase();

    _filteredDocs = _docs.where((d) {
      final typeMatch = _selectedType == null || d['type'] == _selectedType;

      final uid = d['createdBy'] ?? '';
      final userName = userNames[uid]?.toLowerCase() ?? uid.toLowerCase();
      final projectName = (d['projectName'] ?? '').toString().toLowerCase();

      final textMatch =
          f.isEmpty || userName.contains(f) || projectName.contains(f);

      final createdAt =
          DateTime.tryParse(d['createdAt'] ?? '') ?? DateTime.now();
      final fromOk =
          _fromDate == null ||
          createdAt.isAfter(_fromDate!.subtract(Duration(days: 1)));
      final toOk =
          _toDate == null ||
          createdAt.isBefore(_toDate!.add(Duration(days: 1)));
