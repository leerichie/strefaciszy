...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  return showDialog<ProjectLine>(
    ...
    builder: (dialogCtx) => _LineDialogContent(
      existing: existing,
      onNotFound: (code) {
        // Instead of a SnackBar, show an inline error
        // (_LineDialogContent will display it under the TextField)
      },
    ),
  );
}

class _LineDialogContent extends StatefulWidget {
  final ProjectLine? existing;
  final void Function(String rawCode)? onNotFound;

  const _LineDialogContent({
    Key? key,
    this.existing,
    this.onNotFound,
  }) : super(key: key);
  ...
  @override
  State<_LineDialogContent> createState() => _LineDialogContentState();
}

class _LineDialogContentState extends State<_LineDialogContent> {
  late bool isStock;
  late String itemRef, custom;
  late int qty;
  late String unit;
  ...
  void initState() {
    ...
    isStock = e?.isStock ?? true;
    itemRef = e?.itemRef ?? '';
    custom = e?.customName ?? '';
    qty = e?.requestedQty ?? 0;
    unit = e?.unit ?? 'szt';

    if (isStock && itemRef.isNotEmpty) {
      final matched = _stockItems.firstWhere((s) => s.id == itemRef);
      _searchController.text = matched.name;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<bool>(
                  ...
                  items: const [
                    DropdownMenuItem(value: true, child: Text('W Magazynie')),
                ...
                if (isStock) ...[
                  TextFormField(
                    ...
                    validator: (v) {
                      if (isStock && itemRef.isEmpty) return 'Wybierz produkt';
                      return null;
                    },
                  ),
                ...
                // 2b) CUSTOM
                  ...
                  TextFormField(
                    initialValue: custom,
                    decoration: const InputDecoration(labelText: 'Własny prod.'),
                    validator: (v) =>
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: _submit,
          child: Text(widget.existing == null ? 'Dodaj' : 'Zapisz'),
        ),
      ],
    );
  }
  ...
  Future<void> _scanBarcode() async {
    final rawCode = await Navigator.of(context).push<String>(
      MaterialPageRoute(builder: (_) => ScanScreen(returnCode: true)),
    );
    ...
    if (snap.docs.isEmpty) {
      setState(() {
        _errorText = 'Produkt o kodzie $rawCode nie znaleziony';
      });
    } else {
      final doc = snap.docs.first;
      final s = StockItem.fromMap(doc.data()! as Map<String, dynamic>, doc.id);
      setState(() {
        itemRef = s.id;
        _searchController.text = s.name;
        _errorText = null;
