...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  onEdit: (i, newText) async {
    ...
    final editorName = currentUser.displayName ?? currentUser.email ?? '…';
    ...
    // 2) Add a new one with the editor’s name and "now" timestamp
    final newMap = {
      'text': newText,
      'userName': editorName,
    ...
    // 3) Update local state so UI shows the new editor and date instantly
    setState(() {
      _notes[i] = Note(
        text: newText,
        userName: editorName,
        createdAt: DateTime.now(),  // matches serverTimestamp once it returns
      );
    });
  },
                    onDelete: (i) async {
