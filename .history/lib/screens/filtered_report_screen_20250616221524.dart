...
class FilteredReportScreen extends StatelessWidget {
  ...
  Future<void> _exportToExcel(
    ...
    // 1) grab the raw bytes and convert to Uint8List
  final raw = excel.encode()!; 
  final bytes = Uint8List.fromList(raw);

  // 2) make a Polish‐style timestamp
  final now = DateTime.now();
  final formattedDate = DateFormat('dd.MM.yyyy_HH.mm', 'pl_PL').format(now);

  // 3) use the reportType + timestamp as your filename (no “.xlsx” here!)
  ...
  if (savedPath != null) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Zapisano plik: $savedPath.xlsx')),
    );
  } else {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Pobrano plik: $filename.xlsx')),
    );
  }
}