...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _loadAll() async {
    ...
    // 1) If we're editing an existing RW, load its items:
    if (widget.rwId != null) {
      final rwSnap = await projRef
          ...
          .get();
      if (rwSnap.exists) {
        final rwData = rwSnap.data()!;
        _lines = (rwData['items'] as List<dynamic>)
            .map((m) => ProjectLine.fromMap(m))
            .toList();
        _rwExistsToday = true;
        // Optionally pull back _title/_notes if stored on RW doc...
        setState(() {
          _loading = false;
          _initialized = true;
        });
    ...
    // 2) Otherwise, your original draft-vs.-today logic:
