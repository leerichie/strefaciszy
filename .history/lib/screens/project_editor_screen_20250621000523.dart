...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  /// Checks whether an RW (or MM) exists for today via the single-field
  /// `createdDay` filter, avoiding any composite-index requirement.
  Future<void> _checkTodayExists(String type) async {
    final now = DateTime.now();
    final todayStamp = Timestamp.fromDate(
    ...
    final snap = await FirebaseFirestore.instance
        ...
        .where('createdDay', isEqualTo: todayStamp)
        .limit(1)
        .get();
    ...
    setState(() {
      ...
      if (type == 'RW') {
        if (_rwExistsToday && !exists) {
          // If yesterday's RW just vanished, reset previousQty
          _lines = _lines.map((ln) {
            ln.previousQty = 0;
            return ln;
          }).toList();
        }
        _rwExistsToday = exists;
      } else {
        _mmExistsToday = exists;
      }
    });
  }
  ...
  Future<void> _saveRWDocument(String type) async {
    ...
    if (filteredLines.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Dodaj produkty aby wygenerowac RW!')),
      );
      return;
    }
    ...
    try {
      ...
      // ▶️ Catch any error from _checkTodayExists so it doesn’t bubble up
      try {
        await _checkTodayExists(type);
      } catch (e, stack) {
        // This should no longer happen once you update _checkTodayExists,
        // but just in case log it:
        print('Error in _checkTodayExists: $e\n$stack');
      }
    } catch (exception, stackTrace) {
      // Try to unwrap the JS‐interop “converted Future” wrapper:
      dynamic e = exception;
      dynamic s = stackTrace;

      // If it has an inner `.error` and `.stack`, drill in:
      if (exception is Object &&
          (exception as dynamic).error != null &&
          (exception as dynamic).stack != null) {
        e = (exception as dynamic).error;
        s = (exception as dynamic).stack;
      }

      // Now print the unwrapped error & stack:
      debugPrint('❌ Error saving RW: $e');
        ...
        actions: <Widget>[
          IconButton(
            ...
            onPressed: () async {
              ...
              // 2) if the RW‐list screen popped with true (we deleted), reload
              if (didDelete == true) {
                await _loadAll();
                await _checkTodayExists('RW');
              }
                                ...
                                if (_lines.isEmpty) {
                                  ...
                                  if (snap.docs.isNotEmpty) {
                                    ...
                                    for (final item in items) {
                                      ...
                                      final stockRef = FirebaseFirestore
                                          .instance
                                          .collection('stock_items')
                                          .doc(itemId);
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                                        content: Text(
                                          'Przywrócono stan i wyczyszczono projekt i RW',
                                        ),
                                      ),
                                    );
                                    setState(() {
