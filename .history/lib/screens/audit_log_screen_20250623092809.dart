...
class AuditLogScreen extends StatelessWidget {
  ...
  Widget _buildActionRow(BuildContext context, Map<String, dynamic> data) {
    ...
    details.forEach((key, val) {
      // render each detail on its own indented line
      detailLines.add(
        Padding(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Historia – RW według użytkownika')),
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (ctx, snap) {
          ...
          for (var entry in docs) {
            final user = entry['userName'] as String? ?? '—';
            byUser.putIfAbsent(user, () => []).add(entry);
          }

          // Build one ExpansionTile per user
          return ListView(
