...
class _CustomerDetailScreenState extends State<CustomerDetailScreen> {
  ...
  void initState() {
    super.initState();
    _customerRef = FirebaseFirestore.instance.collection('customers').doc(widget.customerId);
                  ...
                  children: [
                    Expanded(
                      child: Text(
                        startDate == null
                            ? 'Start Date'
                            : DateFormat('dd.MM.yyyy', 'pl_PL').format(startDate),
                      ),
                    ),
                    TextButton(
                      child: Text('Select'),
                    ...
                    Expanded(
                      child: Text(
                        estimatedEndDate == null
                            ? 'Estimated End Date'
                            : DateFormat('dd.MM.yyyy', 'pl_PL').format(estimatedEndDate),
                      ),
                    ),
                    TextButton(
                      child: Text('Select'),
            ...
            ElevatedButton(
              onPressed: () {
                ...
                final data = {
                  ...
                  if (estimatedEndDate != null) 'estimatedEndDate': Timestamp.fromDate(estimatedEndDate!),
                };
    ...
    return Scaffold(
      appBar: AppBar(
        title: FutureBuilder<DocumentSnapshot>(
          ...
          builder: (ctx, snap) {
            if (snap.connectionState != ConnectionState.done) return Text('...');
          ...
          return ListView.separated(
            ...
            itemBuilder: (ctx, i) {
              ...
              final created = ts != null
                  ? DateFormat('dd.MM.yyyy HH:mm', 'pl_PL').format(ts.toDate().toLocal())
                  : '';
              final start = startTs != null
                  ? DateFormat('dd.MM.yyyy', 'pl_PL').format(startTs.toDate().toLocal())
                  : '';
              final end = endTs != null
                  ? DateFormat('dd.MM.yyyy', 'pl_PL').format(endTs.toDate().toLocal())
                  : '';
              final costStr = cost != null ? '${cost.toStringAsFixed(2)} zł' : '';
              final lines = <String>[
                'Status: $status',
