          ...
          children: [
            Padding(
              ...
              child: ElevatedButton(
                onPressed:
                    (_rwLocked || _lines.every((l) => l.requestedQty == 0))
                    ? null
                    : () => _saveRWDocument('RW'),
...
// 2) A single row of “Update RW” on the right just above the bar
persistentFooterButtons: !_rwLocked
    ? [
        Align(
          alignment: Alignment.centerRight,
          child: ElevatedButton(
            onPressed:
                _lines.every((l) => l.requestedQty == 0) ? null : () => _saveRWDocument('RW'),
            child: Text(_rwExistsToday ? 'Update RW' : 'Zapisz RW'),
          ),
        ),
      ]
...
// 3) Your bottom nav unchanged (inventory on left, scan on right)
bottomNavigationBar: SafeArea(
  child: BottomAppBar(
    shape: const CircularNotchedRectangle(),
    notchMargin: 6,
      ...
      child: Row(
        ...
        children: [
          IconButton(
            ...
            onPressed: () => Navigator.of(context).push(
              MaterialPageRoute(
                  builder: (_) => InventoryListScreen(isAdmin: widget.isAdmin)),
            ),
          ),
          IconButton(
            tooltip: 'Skanuj',
