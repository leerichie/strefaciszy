    ...
    return Scaffold(
      ...
      floatingActionButton: _rwLocked
          ...
          : FloatingActionButton(
              ...
              onPressed: () async {
                // 1) detect if this item already exists
                //    (we’ll look it up *after* they pick which product)
                final pick = await showProjectLineDialog(context, _stockItems);
                if (pick == null) return;

                // 2) find index in the list
                final existingIndex = pick.isStock
                    ? _lines.indexWhere(
                        (l) => l.isStock && l.itemRef == pick.itemRef,
                      )
                    : _lines.indexWhere(
                        (l) =>
                            !l.isStock &&
                            l.customName.toLowerCase() ==
                                pick.customName.toLowerCase(),
                      );

                // 3a) if it exists: re-open the dialog with the existing line
                ProjectLine? updated;
                if (existingIndex != -1) {
                  updated = await showProjectLineDialog(
                    context,
                    _stockItems,
                ...
                // 3b) otherwise this is brand-new
                else {
                  setState(() => _lines.add(pick));
                }

                // 4) save & sync just like before
                try {
                  await _saveRWDocument('RW');
                } catch (e) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('Auto Sync RW nie powiodło się: $e'),
                    ),
                  );
                }
              },
