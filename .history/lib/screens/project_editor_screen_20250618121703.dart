...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _saveRWDocument(String type) async {
    ...
    // DEBUG: print out the current status
    final projectSnap = await projectRef.get();
    final currentStatus = projectSnap.data()?['status'] as String? ?? 'draft';

    if (currentStatus == type) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Projekt już zapisany jako $type.')),
      );
      return;
    }

    final rwRef = db.collection('rw_documents').doc();
    ...
    try {
      await db.runTransaction((tx) async {
        for (final ln in _lines.where((l) => l.isStock)) {
          ...
          final currentQty = (stockSnap.data()!['quantity'] as int);
          debugPrint(
            '  → stock before: ${stockSnap.data()!['name']} = $currentQty',
          );
          if (ln.requestedQty > currentQty) {
            throw Exception('Za mało ${stockSnap.data()!['name']} w magazynie');
          }
          tx.update(stockRef, {
            'quantity': currentQty - ln.requestedQty,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Zapisano $type i odjęto ilości z magazynu')),
      );
...
class ProjectLine {
  ...
  factory ProjectLine.fromMap(Map<String, dynamic> m) {
    // for loaded-from-Firestore lines we won’t have originalStock stored,
    // so we can default it to requestedQty (so remaining = 0) or fetch it anew
    return ProjectLine(
      ...
      originalStock:
          m['originalStock'] as int? ?? (m['requestedQty'] as int? ?? 0),
    );
  }
  ...
  Map<String, dynamic> toMap() {
    final map = {
      ...
      'unit': unit,
      'originalStock': originalStock, // store it
    };
    if (isStock) {
      map['itemRef'] = itemRef;
    } else {
