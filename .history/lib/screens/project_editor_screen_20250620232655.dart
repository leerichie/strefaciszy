...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  /// Saves (or updates) today’s RW in a single transaction, using
  /// `createdDay` to detect whether to create or update.
  Future<void> _saveRWDocument(String type) async {
    if (!_formKey.currentState!.validate()) return;
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;

    // Only keep lines with qty > 0
    ...
    final now = DateTime.now();
    // For determining whether an RW exists today:
    final todayStamp = Timestamp.fromDate(
      DateTime(now.year, now.month, now.day),
    );
    ...
    // If editing an *existing* RW from before today, block non-admins
    final startOfDay = DateTime(now.year, now.month, now.day);
    ...
    // Preserve original createdAt/createdBy if updating
    ...
    // Build the RW document map (this already injects createdDay)
