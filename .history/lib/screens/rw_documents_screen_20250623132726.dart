...
class _RWDocumentsScreenState extends State<RWDocumentsScreen> {
  ...
  Future<void> _exportCsv(
    ...
    // 1) Build your TSV text
    final csvText = _buildCsv(
      data,
      userNames,
    ); // your existing tab-delimited builder

    // 2) Pick a system temp or documents directory (mobile only)
    Directory dir;
    if (kIsWeb) {
      // Web can’t write a file, so fall back to clipboard:
      await Clipboard.setData(ClipboardData(text: csvText));
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('TSV skopiowane do schowka')),
      );
      return;
    } else {
    ...
    final rawName = data['projectName'] as String? ?? 'dokument';
    final safeName = rawName.replaceAll(RegExp(r'\s+'), '_');
    final stamp = DateFormat(
      'dd.MM.yyyy_HH.mm',
      'pl_PL',
    ).format(DateTime.now());
    final fullName = '$safeName\_$stamp.csv';
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Zapisano plik: $fullName')));

    // 6) Open in default text editor (Notepad, etc.)
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () => _exportCsv(context, data, userNames),
            child: Text('Exportuj CSV'),
          ),
          TextButton(
