...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _saveRWDocument(String type) async {
    ...
    try {
      ...
      // 1) Collect only the lines whose quantity actually changed:
      final changedLines = filteredLines.where((ln) {
        final prev = ln.previousQty ?? 0;
        return ln.requestedQty - prev != 0;
      }).toList();

      // 2) For each changed line, deduct exactly (new–old) from stock
      for (final ln in changedLines) {
        final prev = ln.previousQty ?? 0;
        final diff = ln.requestedQty - prev; // e.g. 2 – 1 = 1
        ...
        // immediately update your in-memory model so next time diff is correct
        ln.previousQty = ln.requestedQty;
      }

      // 3) Commit all the stock updates in one go
      await batch.commit();

      // 4) Now write one audit‐log entry per changed line, using that same diff
      for (final ln in changedLines) {
        final prev = (ln.previousQty! - (ln.requestedQty - ln.previousQty!));
        // (that arithmetic recovers the old prev, if you need it in a pinch)
        final diffText =
            '${ln.requestedQty - prev > 0 ? '+' : ''}'
            '${ln.requestedQty - prev}${ln.unit}';
