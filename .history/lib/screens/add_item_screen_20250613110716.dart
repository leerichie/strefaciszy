...
class _AddItemScreenState extends State<AddItemScreen> {
  ...
  Future<void> _pickAndUploadImage(String docId) async {
    ...
    try {
      ...
      final ref = _storageRef.child('stock_images/$docId.jpg');
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final docRef = await col.add({
        ...
        'updatedBy': uid,
        // itemIndex will be set by your Cloud Function
      });

      // If user picked an image, upload it now
      if (_imageUrl == null) {
        // no image to upload
      } else {
        // uploadImage has already set _imageUrl
      }

      // If they tapped “Pick Photo” after save, we might need to upload now:
      // But typically you invoke pick/upload before save
            ...
            : Form(
                ...
                child: ListView(
                  children: [
                    ...
                    ElevatedButton.icon(
                      onPressed: () async {
                        // we need a docId to upload under → generate one:
                        final fakeId = FirebaseFirestore.instance
                            .collection('stock_items')
                            .doc()
                            .id;
                        await _pickAndUploadImage(fakeId);
                      },
