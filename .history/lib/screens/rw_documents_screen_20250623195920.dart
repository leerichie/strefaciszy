...
class _RWDocumentsScreenState extends State<RWDocumentsScreen> {
              ...
              builder: (ctx, snap) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    // ensure we have the userName loaded
                    final uid = d['createdBy'] as String? ?? '';
                    _fetchUserName(uid);

                    // determine if this document was created today
                    ...
                    // everyone can edit today's docs
                    if (isToday) {
                      actions.add(
                        IconButton(
  ...
  Future<void> _showDetailsDialog(
    ...
    // 2) Figure out the creation timestamp exactly as before
    DateTime dt;
    ...
    // 3) Grab the UID, then fetch that user’s document right here
    final uid = data['createdBy'] as String? ?? '';
    String displayName;

    // First see if we've already cached it
    if (userNames.containsKey(uid)) {
      displayName = userNames[uid]!;
    } else {
      // otherwise grab it live
      try {
