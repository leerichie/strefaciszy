...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
  ...
  Future<void> _saveRWDocument(String type) async {
    ...
    try {
      ...
      // 1) Always safe: just print the exception and stack
      debugPrint('🔥 Error saving RW: $exception');
      debugPrint('📋 Stack trace:\n$stackTrace');

      // 2) Show a generic SnackBar
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Błąd zapisu: ${exception.toString()}')),
      );
    } finally {
      setState(() => _saving = false);
    }
