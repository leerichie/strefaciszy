...
class _RWDocumentsScreenState extends State<RWDocumentsScreen> {
  ...
  Future<void> _exportToExcel(
    ...
    // Add headers
    sheet.getRangeByName('A1').setText('Typ Dok:');
    ...
    sheet.getRangeByName('A2:E2').cellStyle.italic = true;

    // Items
    final startRow = 4;
    sheet.getRangeByName('A$startRow').setText('Materiał:');
    sheet.getRangeByName('B$startRow').setText('Ilość:');
    sheet.getRangeByName('C$startRow').setText('Jednostka:');
    sheet.getRangeByName('A$startRow:B$startRow').cellStyle.bold = true;

    int row = startRow + 1;
    for (final item in (data['items'] as List<dynamic>? ?? [])) {
      sheet.getRangeByName('A$row').setText(item['name'] ?? '');

      final quantityCell = sheet.getRangeByName('B$row');
      quantityCell.setText('${item['quantity'] ?? ''}');
      quantityCell.cellStyle.hAlign = xlsio.HAlignType.right;

      sheet.getRangeByName('C$row').setText(item['unit'] ?? '');
      row++;
    }
