...
exports.updateUserCredentialsHttp = functions.https.onRequest(async (req, res) => {
  if (req.method === 'OPTIONS') {
    return res.status(204).set(corsHeaders).send('');
  }
  res.set(corsHeaders);
  ...
  if (!uid || (!email && !password)) {
    return res.status(400).json({error: 'Missing uid or fields to update'});
  }
  ...
  try {
    ...
    return res.json({uid, email, password: password ? 'updated' : 'unchanged'});
  } catch (e) {
